...
def mutation_function(offspring, ga_instance):
    ...
    for chromosome_idx in range(offspring.shape[0]):
        ...
        genes_to_mutate = np.random.random(size=offspring.shape[1]) < MUTATION_RATE/100
...
ga_instance = pygad.GA(
    ...
    gene_space=[(0, 95) for _ in range(len(weights))]  # Tous les gènes doivent être entre 0 et 95
)
...
def run_genetic_algorithm():
    ...
    fitness_scores = [sum(min(95, sol[i]) * weights[i] for i in range(len(weights))) for sol in final_population]
    ...
    best_candidates = [
        {
            ...
            "criteria": [min(95, value) for value in final_population[sorted_indices[i]].tolist()]
        }
