...
def fitness_function(ga_instance, solution, solution_idx):
    ...
    team_score = sum(
        min(95, solution[i]) * weights[i % len(weights)] for i in range(len(solution))
    )
...
def create_initial_team_population():
    ...
    for _ in range(POPULATION_SIZE):
        selected_indices = np.random.choice(
            sorted_indices[:20], size=TEAM_SIZE, replace=False
        )
        team = []
        ...
        improvements.append(
            {
                ...
                "total_improvement": round(
                    float(
                        sum(
                            opt - orig
                            for orig, opt in zip(original_criteria, optimized_criteria)
                        )
                    ),
                    2,
                ),
...
def run_team_genetic_algorithm():
    ...
    for i in range(TEAM_SIZE):
        ...
        distances = [
            np.linalg.norm(candidate_criteria - candidate) for candidate in population
        ]
        ...
        team_candidates.append(
            {
                ...
                "fitness": round(
                    float(
                        sum(
                            min(95, value) * weights[j]
                            for j, value in enumerate(candidate_criteria)
                        )
                    ),
                    2,
                ),
